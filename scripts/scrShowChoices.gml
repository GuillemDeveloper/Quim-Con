...
for(i = 1; i < array_length_2d(messageGiver.myDialogue, index1); ++i) {
    ...
    choiceBox[i - 1] = instance_create((i * 140), view_hport[view_current] / 1.2, objChoiceBox); //Create the choiceBox and set it in the array
    //With allows us quick access to choiceBox
    with(choiceBox[i - 1]) {
        maxLength = sprite_width - 20; //Set the max length of the message
        maxHeight = sprite_height - 20; //Sett he max height of the message
